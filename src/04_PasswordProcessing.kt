...
val ALL_FIELDS = mapOf<String, (String) -> Boolean>(
    Pair("byr", { str -> (1920..2002).contains(str.toInt()) }),
    Pair("iyr", { str -> (2010..2020).contains(str.toInt()) }),
    Pair("eyr", { str -> (2020..2030).contains(str.toInt()) }),
    Pair("hgt", ::validHeight),
    Pair("hcl", { str -> "#[0-9a-f]{6}".toRegex().matches(str) }),
    Pair("ecl", { str -> EYES.contains(str) }),
    Pair("pid", { str -> "[0-9]{9}".toRegex().matches(str) })
)

val VALID_FIELDS = ALL_FIELDS - "cid"
val EMPTY_LINE : (String) -> Boolean = { it.isEmpty() }

data class Passport(val fields : Map<String, String>)

fun Passport.isValid() = VALID_FIELDS.all { (field, test) -> fields[field]?.let(test) ?: false }
fun validHeight(value : String) = "([0-9]+)(cm|in)".toRegex().let {
    it.matchEntire(value)?.destructured?.let { (size, measure) -> (if (measure == "cm") 150..193 else 59..76).contains(size.toInt()) } ?: false
}

fun toFields(line: String) = line
    .split(" ")
    .map { it.split(":").let { (key, value) -> Pair(key, value) } }
    .toMap()
