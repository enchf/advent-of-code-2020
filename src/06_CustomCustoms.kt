 ...
 * What is the sum of those counts?
 */
fun distinctLetters(list: List<String>) = list
        .flatMap(String::breakInLetters)
        .distinct()
        .size

fun String.breakInLetters() = split("").filterNot(EMPTY_LINE)

fun allSayYes(list: List<String>) = list
        .map(String::breakInLetters)
        .reduce { acc, current -> acc.intersect(current).toList() }
        .size

val transformations = listOf(::distinctLetters, ::allSayYes)

fun main() = fileLines("src/06_CustomCustoms.txt", "src/06_Sample.txt") { it }
        .map { it.chunkWhen(String::isEmpty) }
        .map { transformations.map { f -> it.sumBy(f) } }
        .forEach(::println)
