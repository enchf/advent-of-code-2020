 ...
 * You aren't sure which seat is yours, and all of the flight attendants are busy
 * with the flood of people that suddenly made it through passport control.
 *
 * You write a quick program to use your phone's camera to scan all
 * of the nearby boarding passes (your puzzle input);
 ...
 */
fun rowNumber(path: String, ones: Char, zeros: Char) = path
        .fold("") { acc, it -> acc + if (it == ones) "1" else if (it == zeros) "0" else "" }
        .toInt(2)

fun seatID(pass: String) =
        pass.substring(0..6).let { rowNumber(it, 'B', 'F') * 8 } +
        rowNumber(pass.substring(7), 'R', 'L')

fun mySeat(passes: List<Int>) = (0..(128*8)).minus(passes)

fun main() = fileLines("src/05_BinaryBoarding.txt", "src/05_Sample.txt") { it }
        .map { it.map(::seatID) }
        .onEach { println(it.max()!!) } // Part 1
        .forEach { println(mySeat(it)) } // Part 2
