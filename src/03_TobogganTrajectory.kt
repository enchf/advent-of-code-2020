...
data class TreeLine(val pattern: String)

fun TreeLine.hasTree(position: Int) = pattern[position % pattern.length] == '#'

fun generator(limit: Int, nextX: IntFun, nextY: IntFun) =
    generateSequence(Pair(0, 0)) { Pair(nextX.invoke(it.first), nextY.invoke(it.second)) }
        .takeWhile { it.second < limit }
        .toList()

val slopesX = listOf<IntFun>(
    { it + 1 },
    { it + 3 },
    { it + 5 },
    { it + 7 },
...
fun main() = fileLines("src/03_TobogganTrajectory.txt", "src/03_Sample.txt") { TreeLine(it) }
    .forEach { list ->
        ...
        // Part 2
        allSlopes
            .map { generator(list.size, it.first, it.second) }
            .map { it.count { (x, y) -> list[y].hasTree(x) }.toLong() }
            .reduce(Long::times)
            .let(::println)
    }
